...
function fetch_product_from_api($product_id){
	$url = "https://api.sinaliteuppy.com/product/".$product_id."/6";
	return get_from_api($url);
}
function fetch_variant_from_product($product_id)
{
	$url = "https://api.sinaliteuppy.com/variants/".$product_id;
	return get_from_api($url);
}
...
function update_products_from_sinalite() {
	...
	if( isset( $_GET['update_products_sinalite'] ) ) {
		...
		if(empty($db_time)) {
			$check_24_hour_time = time();
		} else {
			$check_24_hour_time = strtotime('+1 day', $db_time);
		}
		if(time() >= $check_24_hour_time) {
			...
			$wpdb->query("DELETE FROM {$wpdb->prefix}sinalite_products WHERE `update_prod`= 1");
		...
		$get_products_variant_price = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}sinalite_products_variant_price WHERE update_prod = 0 LIMIT 300", OBJECT );
...
function my_admin_page_contents() {
    ?>
    <h1> <?php esc_html_e( 'Sinalite Price setting.', 'my-plugin-textdomain' ); ?> </h1>
...
function my_setting_markup() {
    ?>
    <input type="text" id="sinalite_price_percentage" name="sinalite_price_percentage" value="<?php echo get_option( 'sinalite_price_percentage' ); ?>">
    <?php
}
?>
